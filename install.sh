...
mkdir -p "/var/opendnssec/rpc"


# Install main-directory documentation
#
ls -1 -d README* readme* *.md *.MD *.pdf *.PDF *.txt *.TXT man doc/* 2>/dev/null | \
	sort | uniq | \
	while read f
	do
		if [ -n "$f" ]
		then
			mkdir -p "$PREFIX/doc/ods-rpc"
			echo cp -pr "$f" "$PREFIX/doc/ods-rpc"
			     cp -pr "$f" "$PREFIX/doc/ods-rpc"
		fi
	done
...
#
for d in src contrib/systemd
do
	for f in "$d"/*
	do
		case "$f" in
		*.py|*.pyc|*.pyo)
			mkdir -p "$PREFIX/lib/ods-rpc"
			echo cp -p "$f" "$PREFIX/lib/ods-rpc"
			     cp -p "$f" "$PREFIX/lib/ods-rpc"
			;;
		*.service|*.target)
			case "$HAVE_SYSTEMD" in
			yes)
			...
			*)
			...
			;;
		README*|readme*|*.md|*.MD|*.pdf|*.PDF|*.txt|*.TXT)
...
echo 'Libraries need PYTHONPATH="$PYTHONPATH:'"$PREFIX"'/lib/ods-rpc"'
echo
